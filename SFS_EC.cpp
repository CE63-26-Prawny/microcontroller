...
float SFS_EC::readEC(float voltage, float temperature)
{
...
byte SFS_EC::calibrationWifi(float voltage, float temperature, byte mode)
{
    ...
    switch(mode){
        ...
        if(enterCalibrationFlag){
            if((this->_rawEC<1.9)){                         //recognize 1.413us/cm buffer solution
                compECsolution = 1.413*(1.0+0.0185*(this->_temperature-25.0));  //temperature compensation
            }else if((this->_rawEC<16.8)){                    //recognize 12.88ms/cm buffer solution
            ...
            //if((KValueTemp>0.2) && (KValueTemp<1.5)){
                Serial.println();
        ...
        if(enterCalibrationFlag){
                ...
                if(ecCalibrationFinish){   
                    if((this->_rawEC<1.9)){
                        EEPROM_write(KVALUEADDR, this->_kvalueLow);
                        EEPROM.commit();
                        Serial.print(F(">>>Calibration & Stored Successful"));
...
void SFS_EC::calibration(float voltage, float temperature,char* cmd)
{   
    this->_voltage = voltage;
    this->_temperature = temperature;
    strupr(cmd);
...
void SFS_EC::calibration(float voltage, float temperature)
{   
    this->_voltage = voltage;
    this->_temperature = temperature;
...
boolean SFS_EC::cmdSerialDataAvailable()
{
...
byte SFS_EC::cmdParse(const char* cmd)
{
    byte modeIndex = 0;
...
void SFS_EC::ecCalibration(byte mode)
{
